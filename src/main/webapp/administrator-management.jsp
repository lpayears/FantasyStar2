            ...
            <div class="modal-body">
                <form class="form-horizontal" id="add-form">
                    <div class="form-group">
                        <label for="add-adminName" class="col-sm-2 control-label">账号</label>
                        <div class="col-sm-10">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form class="form-horizontal" id="edit-form">
                    <div class="form-group">
                        <label for="edit-adminName" class="col-sm-2 control-label">账号</label>
                        <div class="col-sm-10">
                            <input type="text" class="form-control" name="adminName"
                                   id="edit-adminName"> <input type="hidden" name="salt"
                                                               id="salt">
                        </div>
                    </div>
                    <div class="form-group">
...
<div class="wrap-fluid">
    <div class="container-fluid paper-wrap bevel tlbr">
        ...
        <ul id="breadcrumb">
            ...
            <li><a href="#" title="Sample page 1">管理员管理</a></li>
                    ...
                    <div class="nest" id="tabletreeClose">
                        ...
                        <div class="body-nest" id="tabletree">
                            ...
                            <form class="form-inline" id="search-form">
                                ...
                                <div class="form-group">
                                    <label for="adminName" class="sr-only">Password</label> <input
                            ...
                            <table id="example-advanced" class="admin-table">
...
<script type="text/javascript">
    ...
    function build_table(list) {
        ...
        $.each(list, function (index, item) {
            ...
            $('<tr></tr>').append(idTd).append(adminNameTd).append(pwdTd)
                .append(nameTd).append(genderTd).append(phoneTd)
                .append(mailTd).append(statusTd).append(btnTd)
    ...
    $(document).on(
        ...
        function () {
            $.ajax({
                url: baseUrl + '/getAdmin/' + $(this).attr('edit-id'),
                method: 'get',
                success: function (result) {
                    var adminData = result.info.admin;
                    $('#edit-adminName').val(adminData.adminName);
                    $('#edit-pwd').val(adminData.password);
                    $('#edit-name').val(adminData.name);
                    //通过css选择器选择
                    $('#edit-modal input[name=gender]').val(
                        [adminData.gender]);
                    $('#edit-phone').val(adminData.cellphone);
                    $('#edit-email').val(adminData.email);
                    $('#edit-modal input[name=status]').val(
                        [adminData.status]);

                    $('#salt').val(adminData.salt);
                }
            })
            $('#edit-modal').modal({
                backdrop: 'static'
    ...
    $('#update-btn').click(function () {
        $.ajax({
            url: baseUrl + '/updateAdmin/' + $(this).attr('edit-id'),
            method: 'put',
    ...
    $('#reset-pwd-btn').click(
        function () {
            var adminName = $('#edit-adminName').val();
            if (confirm('确定要重置[' + adminName + ']的密码吗?')) {
                $.ajax({
                    url: baseUrl + '/resetAdminPwd/'
                        + $(this).attr('reset-id'),
                    method: 'put',
                    data: {
                        'salt': $('#salt').val(),
    ...
    $(document).on('click', '.del-btn', function () {
        var adminName = $(this).parents('tr').find('td:eq(0)').text();
        var adminId = $(this).attr('del-id');
        if (confirm('确定要删除管理员[' + adminName + ']吗?')) {
            $.ajax({
                url: baseUrl + '/delAdmin/' + adminId,
                method: 'delete',
                async: false,
                success: function (result) {
                    if (result.code == 200) {
